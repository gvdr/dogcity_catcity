...
# ╔═╡ ab888cbb-15c4-4d36-991a-6c8afef277ea
begin
	using Dates
end

# ╔═╡ 2dcb8c7d-9f6e-44ea-9198-bb5773dbea00
using DataPipes # a bit experimental, Chain is the standard

# ╔═╡ 15d5cad9-107a-4fa1-bc33-6f16ce5b0e10
using Graphs, WGLMakie, GraphMakie

# ╔═╡ e88cd532-75b9-4378-8a64-33b40d613678
using Makie.Colors

# ╔═╡ 40bede1b-ae77-4364-80b7-aeb877c73844
using GeometricFlux

# ╔═╡ 6e4a50be-4b7e-49e5-b5a2-7a7550f0b9d2
using LinearAlgebra

# ╔═╡ 6d6b642a-680d-477d-9c75-0072697b3197
using DotProductGraphs

# ╔═╡ 1724eae9-a2e3-4357-901d-2178625c23ad
using Clustering

# ╔═╡ eb07b852-8b3b-11ee-20b1-3570d4fa6d5d
using PlutoUI, PlutoTeachingTools

# ╔═╡ 8244c6c0-aea3-4259-b5e9-42809f33a280
ChooseDisplayMode()

# ╔═╡ 7817cc2f-2645-43f9-a2b3-08cd03b8e4ab
md"""
# Julia for complex Networks and Spatial Data
...
# ╔═╡ 980c518f-d103-4d94-b116-dc060ed3120e
md"## Network Data"

# ╔═╡ ef5b89f2-dccc-4013-947a-71910d87a3b5
md"""
°°Hello world°° -> the Zachary's Karate club
"""

# ╔═╡ 4ccd79d8-f2a9-4423-803a-fc6a57c6a0c4
md"We will load some libraries, to show different things"

# ╔═╡ 5ebb0473-66a5-4442-81ac-f62338ca5ad9
md"Julia has both native and very powerful ad hoc piping. Here we'll use **Datapipes**, but there are many other who are great (see MethodChains or Chain)."
...
# ╔═╡ f3e72a5b-d14d-49f3-b5a0-8c048d3e6ff5
md"""## """

# ╔═╡ 64a2563d-eed4-4e8b-954f-8581294f907c
adjacency_matrix(karate,Bool)

# ╔═╡ 8c3ba461-7df8-4550-8d81-a4d2d371f4cc
md"""
## 
...
# ╔═╡ 3d60d3d8-e5af-41b0-89ed-89783c5b2f20
# when friends divide...
clusters = [1, 1, 1, 1, 1, 1, 1, 1, 2, 2, 1, 1, 1, 1, 2, 2, 1, 1, 2, 1, 2, 1, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2];

# ╔═╡ 39572662-1008-444e-9923-fa5b7c096855
md"""## """

# ╔═╡ 3a29585c-c3c2-425f-840a-a808a0ff48a4
n_to_col(i) = i == 1 ? colorant"deepskyblue" : colorant"maroon1";

# ╔═╡ cbb58032-f253-444e-a3ec-9f85d5c43bef
graphplot(karate,
	node_color = clusters .|> n_to_col,
	edge_color = colorant"orange",
	node_size  = 5 .+ degree(karate)) 

# ╔═╡ 2af727ec-c937-41bc-b033-a53520d39e95
md"""## Embedding"""

# ╔═╡ e26b740d-73f1-4573-80be-f4aa806c2e8b
md"""
a map from nodes (or edges) to point in space
"""

# ╔═╡ 4ae65ff3-40c7-45d2-a775-90f5e092c35d
karate2vec = @p karate |>
	FeaturedGraph |>
	node2vec

# ╔═╡ 0dd3deae-9d4d-46ad-8362-f20f99b8a5ec
md""" ## """

# ╔═╡ fc0e44e9-7633-4c1c-a614-67e631c561a3
scatter(karate2vec[1,:],karate2vec[2,:],
	color = clusters .|> n_to_col,
	markersize = 5 .+ collect(degree(karate)))

# ╔═╡ b0f1016d-4bce-490c-82e4-609a3eb169ff
md"""## Random Dot Product Graph"""

# ╔═╡ d6dd901f-43ab-4e91-9d8c-3477fba82f0e
karate_adj = @p let
	karate
	adjacency_matrix
	Matrix
...
# ╔═╡ f3f625f8-3048-4fda-9590-48b7d589d281
begin
	# fix embedding dimension d
	d = 3
	Lₑ, Rₑ = svd_embedding(karate_adj, d)
end;

# ╔═╡ 6780db5e-7c09-4f49-8b58-9eff589db3e7
md"""## """

# ╔═╡ 9ffe4cf6-896a-4798-883f-a1130d1951b7
scatter(Lₑ[:,1],Lₑ[:,2],
	color = clusters .|> n_to_col,
	markersize = 5 .+ collect(degree(karate)))

# ╔═╡ 72f44ec5-1b51-4d88-bab4-18f8ec6f8592
md"""## """

# ╔═╡ 25d7e6f7-d441-4ad9-8568-b10eb6e116af
# reconstructed = Lₑ * Rₑ'
reconstructed = dot_product(Lₑ,Rₑ)

# ╔═╡ 5be99ca7-9b86-4d5b-ab2d-2f229dfe6362
heatmap(reconstructed)

# ╔═╡ e613b6d3-9e61-47eb-94ec-539488058798
heatmap(karate_adj)

# ╔═╡ 8ee7b477-4239-4d44-b14d-c067ba737693
md"""## """

# ╔═╡ 10c89431-8f3d-4a48-b95d-8edf1cfcd398
md"""
embedding dimension = $(@bind td Scrubbable(3))
"""

# ╔═╡ 1a73afa4-cc55-43bf-b8d8-ce9cbda1aa92
function plot_trec(d::Int)
	tL, tR = svd_embedding(karate_adj, td)
	trec = dot_product(tL,tR)
	f = heatmap(trec)
	return f
end

# ╔═╡ 4dad31ad-acdd-4c73-8849-ab1e040cba48
plot_trec(td)

# ╔═╡ 6fc653c1-0b17-45ff-a208-2a2a5b3e541b
md"""## cluster"""

# ╔═╡ d64bed7b-fc2d-443f-bbc2-122897c7c05e
md""" rediscovering the groups """

# ╔═╡ 8b59869d-ef7a-4809-93d5-81db9f805efc
results_n2v = kmeans(karate2vec,2) |> assignments

# ╔═╡ c3d4cb4f-7267-430e-9082-9c4eac0e9693
graphplot(karate,
	node_color = results_n2v .|> n_to_col,
	edge_color = colorant"orange",
	node_size  = 5 .+ degree(karate))

# ╔═╡ d182f82a-4194-4e59-a399-9f1537fdf79e
md""" ## """

# ╔═╡ 2c92bc16-17d3-4d2c-810f-72581c6dac0c
karateRDPG = hcat(Lₑ,Rₑ)'

# ╔═╡ 9f2c941d-2b10-44bb-b8ee-6d307d25e58a
results_rdpg = kmeans(karateRDPG,2) |> assignments

# ╔═╡ 3438ceda-a701-4608-80a2-34a3ef9c38c8
graphplot(karate,
	node_color = results_rdpg .|> n_to_col,
	edge_color = colorant"orange",
	node_size  = 5 .+ degree(karate)) 

# ╔═╡ bb644a3b-aecc-4747-99a5-7cdab65c662b
md""" ##  """
...
julia_version = "1.9.4"
manifest_format = "2.0"
project_hash = "8217d7f4839b14da24645f735f1148885f1deb06"
...
git-tree-sha1 = "ff8b87eba350739a514ea142415bb0f9f9d671fb"
...
git-tree-sha1 = "d7bebf692fe4e11a401db1ae14648e82c582d908"
...
git-tree-sha1 = "dcd9cd1e485815a1f495e699bc6f6061520d878d"
